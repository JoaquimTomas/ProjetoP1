    ...
    if(ficheiro==NULL) {
        ...
        if(controlo!=1) {
            ...
            if(controlo!=quantVeiculos) {
                ...
                printf("\nGravacao de Veiculos efetuada com sucesso\n\n");
            }
        }
    }
    fclose(ficheiro);
    ...
    if (ficheiro==NULL) {
        ...
        if(controlo!=1) {
            ...
            if(controlo != quantEncomendas) {
                printf("Erro na gravaçao dos dados(lista de Encomendas)");
            } else {
                printf("\n\nGravacao de Encomendas efetuada com sucesso.\n");
...
void guardarFicheiroTextoEncomendas(tipoEncomenda listaEncomendas[MAX_ENCOMENDA], int quantEncomendas) {
    ...
    if(ficheiro==NULL) {
        printf("\n\nERRO: Falha na abertura ou criacao do ficheiro");
...
void lerFicheiroBinarioEncomendas(tipoEncomenda listaEncomendas[MAX_ENCOMENDA], int *quantEncomendas) {
    ...
    if(ficheiro==NULL) {
        ...
        if(controlo != 1) {
            ...
            controlo=fread(listaEncomendas,sizeof(tipoEncomenda), *quantEncomendas,ficheiro);
...
void guardarFicheiroTextoEncDevolv(tipoEncomenda listaEncomendas[MAX_ENCOMENDA],tipoVeiculo listaVeiculos[MAX_VEICULOS],int quantDevol) {
    ...
    ficheiro = fopen("logDevolvidas.txt", "a    ");
    ...
    if(ficheiro == NULL) {
        ...
        for(i=0; i<quantDevol; i++)
        {
            if(listaEncomendas[i].estado == 'D'){

                fprintf(ficheiro, "Registo da encomenda: %d\n", listaEncomendas[i].numeroReg);
                fprintf(ficheiro, "Matricula de veiculo: %s\n", listaEncomendas[i].matriculaEnc);
                fprintf(ficheiro, "Destino: %s\n", listaEncomendas[i].destino);
                fprintf(ficheiro, "Data de devolucao: %d-%d-%d\n",listaEncomendas[i].dataDevol);
                fprintf(ficheiro, "Causa de devolucao: %s\n", listaEncomendas[i].razao);
